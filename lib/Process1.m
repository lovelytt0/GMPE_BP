...
for i=1:length(d(:,1))
    clear data;
    for j=1:3
        [data(:,j),Info(i)]=readKuma(d(i,j).name) ;
        data(:,j)=data(:,j)-mean(data(:,j));
    end
    Data{i}=data;
    lon(i,:)=Info(i).Station.longitude;
    lat(i,:)=Info(i).Station.latitude;
    r(i,3)=Info(i).Station.height/1000;%change into UNIT km
end
...
for i=1:length(d(:,1))
    ...
    [EW_a(i) EW_v(i)  EW_d(i)]= response_spectrum(EWori(i,:)',sample_period,T,damp);
    %NS
    NSori(i,:)=Data{i}(:,2)'/100;
    [NSvaluemax(i) NSlocmax(i)]=max( NSori(i,:));
    [NS_a(i) NS_v(i)  NS_d(i)]= response_spectrum(NSori(i,:)',sample_period,T,damp);
