...
ha=tight_subplot(4,2,[0.03 0.05 ],[0.1 0.1 ],[0.1 0.15])
...
for n_f=2:5
    ...
    for k=kk
        ...
        if equation_flag==1
                ...
                ft.PGA1=ft.PGA1(B1);
                ft.Y1=ft.Y1(B1);
            a1=loglog(ft.X1,ft.PGA1/980.66,'bo');
            hold on
            if isfield(ft,'X2')
                [A2 B2]=sort(ft.X2);
                ft.X2=ft.X2(B2);
                ft.PGA2=ft.PGA2(B2);
ft.Y2=ft.Y2(B2);
            a2=loglog(ft.X2,ft.PGA2/980.66,'ko');
            
            a4=loglog(ft.X2,ft.Y2/980.66,'r','linewidth',2);
            end
            
            
            a3=loglog(ft.X1,ft.Y1/980.66,'g','linewidth',2);
        elseif equation_flag==2
            loglog(X,PGA,'ko');
            hold on;
            loglog(X,Y,'.r')
...
if exist('a2')
h=gridLegend([ a1 a2 a3 a4 ],1,'forearc residual','backarc residual','forearc prediction','backarc prediction','location','east')
else
    h=gridLegend([ a1  a3  ],1,'forearc residual','forearc prediction','location','east');
...
set(ha([2,4,6,8]),'YTickLabel','')
%set(ha([2]),'YTickLabel','')
set(ha(1:6),'XTickLabel','')

set(gcf,'color','white','paperpositionmode','auto');
%figname=strcat(project_ad,'Fig/final.png');
figname=strcat('/home/tian_feng/Workspace/GMPEfig/',path(27:42),'Psa.png');
