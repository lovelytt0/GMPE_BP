...
for p=1:ps
    ...
    for q=1:qs
                dx=111.195*(lat0-ux(p))
        dy=111.195*(lon0-uy(q))*cosd((lat0+ux(p))/2)
              sd=phtime2d(lat0,lon0,dep0,ux(p),uy(q),dep0,r(:,2),r(:,1),rr,dd,tt)-ret.dsx*dx-ret.dsy*dy;% along ray path
